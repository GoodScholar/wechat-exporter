...
export function renderHtml(article) {
  ...
  $('table').wrap('<div class="article-table" role="region" aria-label="文章表格，可左右滚动" tabindex="0"></div>');
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data:; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'">
<title>${escapeHtml(article.title)}</title><style>
body{max-width:760px;margin:48px auto;padding:0 28px;color:#242c29;font:16px/1.85 'PingFang SC','Microsoft YaHei',sans-serif;overflow-wrap:anywhere}h1{font-size:30px;line-height:1.4}header{border-bottom:1px solid #ddd;margin-bottom:28px;padding-bottom:22px}header p,footer{color:#66716a;font-size:13px}a{color:#16724d}img{max-width:100%;height:auto}pre{white-space:pre-wrap;background:#f3f5f3;padding:16px}blockquote{border-left:3px solid #86ab96;margin-left:0;padding-left:20px;color:#56655c}.article-table{overflow-x:auto;margin:1em 0}.article-table:focus-visible{outline:2px solid #16724d;outline-offset:2px}table{border-collapse:collapse;width:100%;overflow-wrap:normal}td,th{border:1px solid #ddd;padding:8px}footer{border-top:1px solid #ddd;margin-top:36px;padding-top:16px}@media print{body{margin:0;padding:0}.article-table{overflow:visible}table{overflow-wrap:anywhere}img,pre,tr{break-inside:avoid}h1,h2,h3{break-after:avoid}img{max-height:200mm;object-fit:contain}}
</style></head><body><header><h1>${escapeHtml(article.title)}</h1><p>${[...new Set([article.account, article.author, article.date].filter(Boolean))].map(escapeHtml).join(' / ')}</p><a href="${escapeHtml(article.url)}">查看原文</a></header><main>${$.html()}</main><footer>来源：${escapeHtml(article.url)}${article.warnings.length ? '<p>' + article.warnings.map(escapeHtml).join('<br>') + '</p>' : ''}</footer></body></html>`;
}
