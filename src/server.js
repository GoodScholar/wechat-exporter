...
export function createApp({ dataDir = path.join(root, '.data'), exporter, interval } = {}) {
  const app = express();
  const verification = createVerificationBrowser(dataDir);
  const store = new JobStore(dataDir, exporter || ((url, formats) => exportArticle(url, formats, {
    getHtml: async url => await verification.read(url) || (await fetchResource(url)).bytes.toString('utf8')
  })), interval);
  ...
  app.get('/api/jobs', (req, res) => res.json(store.list().map(job => ({
    ...job, outputDirectory: path.resolve(store.directory, 'exports')
  }))));
  app.post('/api/jobs', (req, res) => res.status(201).json(store.create(req.body.text, req.body.formats)));
  app.post('/api/jobs/:id/retry', (req, res) => res.json(store.retry(req.params.id)));
  ...
  app.get('/api/items/:id/download', (req, res) => {
    const item = store.list().flatMap(job => job.items).find(item => item.id === req.params.id && item.status === 'success');
    if (!item) return res.status(404).json({ error: '文件尚未生成' });
    res.download(store.file(item), `${safeName(item.title)}.zip`, { dotfiles: 'allow' });
  });
  app.get('/api/jobs/:id/download', (req, res, next) => {
    const job = store.list().find(job => job.id === req.params.id);
    const items = job?.items.filter(item => item.status === 'success');
    if (!items?.length) return res.status(404).json({ error: '此批次还没有可下载文件' });
    const zip = new JSZip();
    for (const item of items) zip.file(`${safeName(item.title)}_${item.id.slice(0, 8)}.zip`, createReadStream(store.file(item)));
